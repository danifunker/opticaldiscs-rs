...
mod tests {
    ...
    fn mac_roman_upper_range() {
        ...
        // 0x9B → õ (U+00F5) — the entry that was historically missing
        assert_eq!(mac_roman_char(0x9B), 'õ');
        // 0x9F → ü (U+00FC), a very common accented char
        assert_eq!(mac_roman_char(0x9F), 'ü');
        // 0xA9 → © (U+00A9), 0xAA → ™ (U+2122)
        assert_eq!(mac_roman_char(0xA9), '©');
        assert_eq!(mac_roman_char(0xAA), '™');
        // 0xFF → ˇ (U+02C7), the last entry (was previously U+FFFD)
        assert_eq!(mac_roman_char(0xFF), '\u{02C7}');
    }
}
